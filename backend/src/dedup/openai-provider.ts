  ...
  async extractFields(question: string): Promise<MarketFactFields> {
    const response = await this.structuredChat(
      ...
      [
        ...
        'Basis is the observation rule such as close, intraday, settlement, average, or official_result.',
      ].join(' '),
      question,
    );
  ...
  async judgeSameFact(
    ...
    const response = await this.structuredChat(
      ...
      [
        ...
        'The supplied structured fields already match and are authoritative.',
        'Judge only remaining semantic details and phrasing.',
